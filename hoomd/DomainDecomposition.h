 ...
 *
 *  One way to perform a domain decomposition is to subdivide the box into equal sized widths along
 * each dimension. To achieve an optimal domain decomposition (i.e. minimal communication costs),
 * the global domain is sub-divided such as to minimize surface area between domains, while
 * utilizing all processors in the MPI communicator.
 *
 *  Alternatively, unequal sized cuts can be taken. This is advantageous for simulations with
 * non-homogeneous particle distributions, e.g., a vapor-liquid interface. The user can specify N-1
 * of the fractions at construction time, provided that the specified fractions must create a grid
 * commensurate with the number of ranks available. The final rank with is chosen so that the total
 * box is covered. If the specified number of ranks does not match the number that is available,
 * behavior is reverted to the normal default with uniform cuts along each dimension.
 *
...
class PYBIND11_EXPORT DomainDecomposition
    ...
    void setCumulativeFractions(unsigned int dir,
    ...
    unsigned int
    placeParticle(const BoxDim& global_box, Scalar3 pos, const unsigned int* cart_ranks);

    //! Get the number of grid cells in each dimension.
    uint3 getGridSize(void) const
        {
        return make_uint3(m_nx, m_ny, m_nz);
        }
    ...
    void subdivide(unsigned int n_node_ranks,
                   Scalar3 L,
                   unsigned int nx,
                   unsigned int ny,
                   unsigned int nz,
